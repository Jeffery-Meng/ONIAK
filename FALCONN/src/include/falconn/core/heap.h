...
// simple k heap used for heaping sort
template <typename KeyType, typename DataType>
class Simple_k_Heap : public HeapBase<KeyType, DataType> {
  int k_;
 public:
  Simple_k_Heap(int k): k_(k) {}
  ...
  void stream(const KeyType& key, const DataType& data) {
    if (this->num_elements_ < k_) {
      this->insert(key, data);
    } else {
      replace_top(key, data);
    }
  }
  ...
  std::vector<typename HeapBase<KeyType, DataType>::Item>&& move_data() {
