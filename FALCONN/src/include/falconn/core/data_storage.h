...
/*
  class WritableSequenceIterator{
    public:
  WritableSequenceIterator(ArrayDataStorage& parent) : parent_(&parent) {
      if (parent_->data_.size() == 0) {
        parent_ = nullptr;
        index_ = -1;
      } else {
    ...
    WritableSequenceIterator() {}

    PointType& get_point() { return parent_->data_[index_]; }
    const PointType& get_point() const { return parent_->data_[index_]; }
   ...
   private:
    int_fast64_t index_ = -1;
    ArrayDataStorage* parent_ = nullptr;
    StdVectorPrefetcher<PointType> prefetcher_;
  };*/
